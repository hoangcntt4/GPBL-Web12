...
namespace Misa.Amis.Infrastructure.Repositories
{
  ...
  public class BaseRepository<T> : IBaseRepository<T> where T : BaseEntity
  {
    ...
    {
      var sql = $"create temporary table filteredTable select * from View_{TableName}{(condition != null ? $" where {condition}" : string.Empty)} order by CreatedDate desc; select * from filteredTable limit @take offset @skip; select count(*) from filteredTable; drop temporary table filteredTable;";//lenh truy van tim kiem va phan trang
      //khoi tao ket noi
